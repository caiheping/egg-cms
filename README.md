...
1. **打开部门列表页面**  
   ...
   - 页面将显示所有已创建的部门信息，包括部门名称、状态、创建时间等。  
2. **分页浏览**  
   - 如果部门数量较多，可通过页面底部的分页控件切换不同页面的数据。  

### 搜索部门
1. **使用搜索框**  
   - 在页面顶部的搜索框中输入部门名称或关键字。  
2. **执行搜索**  
   - 点击 **“搜索”** 按钮，系统将筛选出匹配的部门记录。  
3. **重置搜索**  
   - 点击 **“重置”** 按钮可清除搜索条件，恢复显示全部部门。  

### 添加新部门
1. **进入添加页面**  
   - 点击部门列表右上角的 **“添加部门”** 按钮。  
2. **填写部门信息**  
   - **上级部门**：从下拉菜单中选择该部门的上级（如“总部”）。  
   - **部门名称**：输入部门名称（如“测试一级”）。  
   - **部门状态**：选择“正常”或“停用”。  
3. **保存设置**  
